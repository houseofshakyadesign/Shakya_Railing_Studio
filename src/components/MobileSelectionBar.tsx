...
export function MobileSelectionBar() {
  const { selectedProduct, selectProduct, settings } = useStudio();
    ...
    <AnimatePresence>
      {selectedProduct && !hidden ? (
        <motion.div
          ...
          className="fixed inset-x-0 bottom-0 z-40 border-t border-hairline bg-background/95 shadow-lift backdrop-blur-xl lg:hidden"
        >
          <div className="flex items-center justify-between gap-3 px-5 py-3">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="truncate text-xs font-medium text-foreground">
                  <span className="font-semibold text-bronze">{selectedProduct.code}</span> ·{" "}
            ...
            <Link
              to="/calculator"
              className="flex shrink-0 items-center gap-2 bg-charcoal px-5 py-3 text-[0.68rem] tracking-[0.2em] text-ivory uppercase hover:bg-bronze transition-colors"
            >
