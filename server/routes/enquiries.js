...
function formatEnquiry(e) {
  return {
    ...
    heightFt: Number(e.height_ft) || 3.5,
    estimatedAreaSqft: Number(e.estimated_area_sqft) || 0,
    estimatedPanelQuantity: Number(e.estimated_panel_quantity) || 1,
    standardModuleWidthFt: Number(e.standard_module_width_ft) || 4.0,
    estimatedPrice: Number(e.estimated_price) || 0,
    quantity: Number(e.quantity) || 1,
    area: Number(e.area) || 0,
    totalArea: Number(e.total_area) || 0,
    rate: Number(e.rate) || 0,
    estimatedTotal: Number(e.estimated_total) || 0,
    status: e.status || "new",
  ...
  try {
    ...
    await query(
      `INSERT INTO enquiries (
        id, customer_name, phone, email, location, project_type,
        product_id, product_code, product_name, material, is_custom,
        length_ft, height_ft, estimated_area_sqft, estimated_panel_quantity,
        standard_module_width_ft, estimated_price, quantity, area,
        total_area, rate, estimated_total, status, additional_requirements
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        ...
        e.estimatedPanelQuantity || 1,
        e.standardModuleWidthFt || 4.0,
        e.estimatedPrice || 0,
        e.quantity || 1,
        e.area || 0,
        e.totalArea || 0,
